...
char* s_STSTR_FLYNN = STSTR_FLYNN;
...
boolean deh_GetData(char*, char*, int_64_t*, char**, FILE*);
...
void deh_procThing(DEHFILE* fpin, FILE* fpout, char* line)
{
    char key[DEH_MAXKEYLEN];
    char inbuffer[DEH_BUFFERMAX];
    int_64_t value;      // All deh values are ints or longs
    int indexnum;
    int ix;
        ...
        for (ix = 0; ix < DEH_MOBJINFOMAX; ix++)
        {
			if (!strcasecmp(key, deh_mobjinfo[ix]))  // killough 8/98
            {
				if (!strcasecmp(key, "bits") && !value) // killough 10/98
                {
                    ...
                    if (fpout) fprintf(fpout, "Bits = 0x%08llX = %lld \n",
                        value, value);
                }
...
void deh_procPointer(DEHFILE* fpin, FILE* fpout, char* line) // done
{
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
		...
		if (!strcasecmp(key, deh_state[4]))  // Codep frame (not set in Frame deh block)
        {
            ...
            if (fpout) fprintf(fpout, " - applied %p from codeptr[%lld] to states[%d]\n", deh_codeptr[value], value, indexnum);
            ...
            if (fpout) fprintf(fpout, "Invalid frame pointer index for '%s' at %lld, xref %p\n",
...
void deh_procSounds(DEHFILE* fpin, FILE* fpout, char* line)
{
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
        ...
        if (!strcasecmp(key, deh_sfxinfo[1]))  // Zero/One
            original_S_sfx[indexnum].singularity = value;
        else
        if (!strcasecmp(key, deh_sfxinfo[2]))  // Value
            original_S_sfx[indexnum].priority = value;
        else
        if (!strcasecmp(key, deh_sfxinfo[3]))  // Zero 1
            original_S_sfx[indexnum].link = (sfxinfo_t*)(intptr_t)value;
        else
        if (!strcasecmp(key, deh_sfxinfo[4]))  // Zero 2
            original_S_sfx[indexnum].pitch = value;
        else
        if (!strcasecmp(key, deh_sfxinfo[5]))  // Zero 3
            original_S_sfx[indexnum].volume = value;
        else
        if (!strcasecmp(key, deh_sfxinfo[6]))  // Zero 4
            original_S_sfx[indexnum].data = (void*)(intptr_t)value; // killough 5/3/98: changed cast
        else
        if (!strcasecmp(key, deh_sfxinfo[7]))  // Neg. One 1
            original_S_sfx[indexnum].usefulness = value;
        else
        if (!strcasecmp(key, deh_sfxinfo[8]))  // Neg. One 2
            original_S_sfx[indexnum].lumpnum = value;
        else
        if (fpout) fprintf(fpout, "Invalid sound string index for '%s'\n", key);
    }
    return;
}
...
void deh_procAmmo(DEHFILE* fpin, FILE* fpout, char* line)
{
    ...
    // killough 8/98: allow hex numbers in input:
    sscanf(inbuffer, "%s %lli", key, &indexnum);
    if (fpout) fprintf(fpout, "Processing Ammo at index %lld: %s\n",
        indexnum, key);
    if (indexnum < 0 || indexnum >= NUMAMMO)
        if (fpout) fprintf(fpout, "Bad ammo number %lld of %d\n",
            indexnum, NUMAMMO);
...
void deh_procWeapon(DEHFILE* fpin, FILE* fpout, char* line)
{
    ...
    // killough 8/98: allow hex numbers in input:
    sscanf(inbuffer, "%s %lli", key, &indexnum);
    if (fpout) fprintf(fpout, "Processing Weapon at index %lld: %s\n",
        indexnum, key);
    if (indexnum < 0 || indexnum >= NUMWEAPONS)
        if (fpout) fprintf(fpout, "Bad weapon number %lld of %d\n",
            indexnum, NUMAMMO);
    ...
    if (fromlen == 4 && tolen == 4)
    {
        ...
        if (fromlen < 7 && tolen < 7)  // lengths of music and sfx are 6 or shorter
        {
            ...
            for (i = 1; i < NUMSFX; i++)
            {
                ...
                if (strlen(original_S_sfx[i].name) != fromlen) continue;
				if (!strncasecmp(original_S_sfx[i].name, inbuffer, fromlen))
                {
                    if (fpout) fprintf(fpout,
                        "Changing name of sfx from %s to %*s\n",
                        original_S_sfx[i].name, usedlen, &inbuffer[fromlen]);

                    original_S_sfx[i].name = strdup(&inbuffer[fromlen]);
                    found = true;
...
boolean deh_GetData(char* s, char* k, int_64_t* l, char** strval, FILE* fpout)
{
    char* t;  // current char
    int_64_t val; // to hold value of pair
