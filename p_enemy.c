...
static boolean P_Move(mobj_t *actor) // killough 9/12/98
{
  fixed_t tryx;
  fixed_t tryy;
  boolean try_ok;
...
void A_SpawnObject(mobj_t* actor)
{
    ...
    an = actor->angle + (unsigned int)(((long long)angle << 16) / 360);
    ...
    if (mo->info->flags & (MF_MISSILE))
    {
        ...
        if (actor->info->flags & (MF_MISSILE))
        {
...
void A_MonsterProjectile(mobj_t* actor)
{
    ...
    // adjust angle
    mo->angle += (unsigned int)(((long long)angle << 16) / 360);
    an = mo->angle >> ANGLETOFINESHIFT;
    mo->momx = FixedMul(mo->info->speed, finecosine[an]);
    mo->momy = FixedMul(mo->info->speed, finesine[an]);
